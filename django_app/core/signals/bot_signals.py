# django_app/core/signals/bot_signals.py
...
async def send_run_bot_request(bot_setup):
    fastapi_url = f"{os.getenv('FASTAPI_URL')}/api/bot/run_bot"
    payload = {"token": bot_setup.get_token(), "username": bot_setup.bot_username, "user_id": bot_setup.owner.telegram_id}
    async with httpx.AsyncClient() as client:
        await client.post(fastapi_url, json=payload, timeout=30.0)

async def send_stop_bot_request(bot_setup):
    fastapi_url = f"{os.getenv('FASTAPI_URL')}/api/bot/stop_bot"
    payload = {"user_id": bot_setup.owner.telegram_id, "username": bot_setup.bot_username}
    async with httpx.AsyncClient() as client:
        await client.delete(fastapi_url, json=payload, timeout=30.0)